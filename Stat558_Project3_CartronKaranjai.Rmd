...
data %>% 
  ggplot() +
  aes(x = global_subjectivity, y = shares) +
  geom_point() +
  geom_jitter() +
...
```{r}
Pred_Linear_Test <- predict(mlrWithoutVS, test)
...
postResample(pred = PredPCAtest, obs = pc_test)
```
...
```{r}
rf_pred <- predict(rfFinalModel, test$shares)

postResample(pred = rf_pred, obs = test$shares)
```

## Boosted Trees

```{r}
Boost_pred <- predict(boostedFinalModel, test$shares)
