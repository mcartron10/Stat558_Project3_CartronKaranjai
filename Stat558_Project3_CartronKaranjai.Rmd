...
Let us first understand the pairplots for all the variables explaining keywords.
...
This removes all the highly correlated keyword variables that convey the same information. Moving on to self referenced shares. 
...
Now removing any variables with a correlation higher than 0.8.
...
data
```
We will also remove `is_weekend` from our analysis since the variables `weekday_is_sunday` and `weekday_is_saturday` captures the information from both these variables. 
...
Expected Behavior : Readers prefer titles with lesser number of words. 
...
Expected Behavior : Less images do better. 
...
Expected Behavior : Most number of articles are shared over the weekends.

Polarity is float which lies in the range of [-1,1] where 1 means positive statement and -1 means a negative statement. Do title polarity affect the average shares?
...
Since we are dealing with `r ncol(data)` variables, it is probably important to know that we would need to employ a feature selection technique. Feature selection is the process of reducing the number of input variables when developing a predictive model.
...
As we can see, the Root Mean Square Error of the model is `mlrWithoutVS$results$RMSE`. Now let us see how this changes with PCA. Let us check the model for multicollinearity. 
...
We will now fit a linear regression using these PCs. 
...
Now that we have a Multiple Linear Regression with PCA, let us see how a Lasso Regression on the original datset competes in terms of variable selection. 
